...
const LeaderboardPage: React.FC<LeaderboardPageProps> = ({ onBackToMenu }) => {
  ...
  return (
    <div className="flex flex-col items-center justify-center h-full p-4 w-full max-w-lg mx-auto">
      <h1 className={`text-4xl sm:text-5xl font-bold mb-2 ${COLORS.TEXT_ACCENT} tracking-tight`}>
        GRID <span className={COLORS.TEXT_PRIMARY}>LEGENDS</span>
      </h1>
      <p className={`text-xs ${COLORS.TEXT_MUTED} font-mono mb-4 sm:mb-6 text-center`}>
      ...
      {leaderboard.length === 0 ? (
        ...
        <div className={`w-full rounded-lg shadow-2xl ${COLORS.MODAL_BG} border-2 ${COLORS.BORDER_ACCENT} overflow-y-auto max-h-[55vh]`}> {/* Adjusted max-h slightly for the new text */}
      ...
      <button
        ...
        className={`mt-6 px-8 py-3 text-lg font-semibold ${COLORS.TEXT_PRIMARY} bg-purple-600 hover:bg-purple-500 rounded-lg shadow-xl hover:shadow-purple-500/50 transition-all duration-300 transform hover:scale-105 focus:outline-none focus:ring-4 focus:ring-purple-400 focus:ring-opacity-50`}
      >
        Return to Main Menu
      </button>
    </div>
